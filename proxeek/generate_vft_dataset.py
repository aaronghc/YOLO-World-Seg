...
def create_user_message_with_images(row, virtual_image_url, physical_image_url, property_type):
...
def generate_vft_dataset(max_rows=None):
    """Generate vision fine-tuning dataset from CSV and images
    
    Args:
        max_rows (int, optional): Maximum number of rows to randomly sample from CSV.
                                 If None, use all rows. Default: None
                                 
    Examples:
        generate_vft_dataset()           # Use all ~100 rows
        generate_vft_dataset(50)         # Use random 50 rows
        generate_vft_dataset(20)         # Use random 20 rows (for quick testing)
    """
    ...
    # Load CSV data
    csv_path = os.path.join(script_dir, 'output', 'Results_feed_training.csv')
    ...
    if max_rows is not None and max_rows < total_rows:
        ...
        print(f"🎲 Randomly selected {max_rows} out of {total_rows} rows")
    else:
    ...
    for row_num, row in enumerate(selected_rows, 1):
        ...
        # Create dataset entries for each valid property
        for property_type in valid_properties:
            rating_str = row[property_type].strip()
            rating = int(float(rating_str))  # Convert to int (handles "3.0" -> 3)
            
            # Create user message with images
            user_content = create_user_message_with_images(
            ...
            entry = {
                "messages": [
                    {
                        "role": "system",
                        "content": get_property_system_prompt(property_type)
                    },
                    {
                        "role": "user",
    ...
    print(f"   Generated examples: {len(dataset)}")
    
    return dataset

def split_dataset(dataset, test_per_property=5):
    """Split dataset into training and testing sets"""
    # Group by property type
    by_property = defaultdict(list)
    for entry in dataset:
        by_property[entry['property_type']].append(entry)
    ...
    for property_type, entries in by_property.items():
        # Shuffle entries for this property
        random.shuffle(entries)
        
        # Split into test and train
        test_entries = entries[:test_per_property]
        train_entries = entries[test_per_property:]
        
        test_dataset.extend(test_entries)
        train_dataset.extend(train_entries)
        
        print(f"   {property_type}: {len(train_entries)} train, {len(test_entries)} test")
...
def save_dataset_as_jsonl(dataset, filename):
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        for entry in dataset:
            # Remove extra fields that aren't needed for training
            clean_entry = {
...
def main(max_rows=None):
    ...
    Args:
        max_rows (int, optional): Maximum number of rows to sample from CSV.
                                 If None, use all rows. Default: None
    """
    print("🎯 ProXeek Vision Fine-Tuning Dataset Generation")
    print("=" * 60)
    
    # Generate dataset
    dataset = generate_vft_dataset(max_rows=max_rows)
    ...
    # Split dataset
    print(f"\n🔄 Splitting dataset...")
    train_dataset, test_dataset = split_dataset(dataset, test_per_property=3)
    
    # Save datasets
    save_dataset_as_jsonl(train_dataset, 'vft_property_rating_train.jsonl')
    save_dataset_as_jsonl(test_dataset, 'vft_property_rating_test.jsonl')
    
    print(f"\n🎉 Vision Fine-tuning dataset ready!")
    print(f"   Training examples: {len(train_dataset)}")
    print(f"   Testing examples: {len(test_dataset)}")
    print(f"   Model: gpt-4o-2024-08-06")
    print(f"   Method: Vision Fine-tuning")
